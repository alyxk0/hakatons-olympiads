...
#define GREEN 10 //обозначаем пин для зеленого светодиода(загорается при напряжении левой руки)
#define YELLOW 11 //обозначаем пин для желтого светодиода(загорается при напряжении правой руки)
...
int choice_hands (void) {
  t_choice = millis();
  int flag_choice = 0;
  while ((millis() - t_choice) < 1100) {
    if (sredn_filtr_left() > 225)
      flag_choice |= 1;
    if (sredn_filtr_right() > 225)
      flag_choice |= 2;
  }
  return flag_choice;
}
